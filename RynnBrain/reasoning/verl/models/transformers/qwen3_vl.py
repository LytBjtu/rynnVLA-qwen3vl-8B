...
def get_rope_index(
    ...
    **kwargs,
    ...
    if input_ids is not None and (image_grid_thw is not None or video_grid_thw is not None):
        ...
        if llm_positions.shape[1] != valid_len:
            # Align when input_ids were truncated but grid_thw wasn't.
            if llm_positions.shape[1] > valid_len:
                llm_positions = llm_positions[:, :valid_len]
            else:
                pad_len = valid_len - llm_positions.shape[1]
                if llm_positions.shape[1] > 0:
                    pad_vals = llm_positions[:, -1:].repeat(1, pad_len)
                else:
                    pad_vals = torch.zeros((3, pad_len), dtype=llm_positions.dtype, device=llm_positions.device)
                llm_positions = torch.cat([llm_positions, pad_vals], dim=1)
