...
class PlanningCliClient:
    def __init__(self, server_url: str, llm_type: str, timeout: int = 300) -> None:
        ...
        # Avoid accidentally routing localhost requests through corporate/system proxy.
    ...
    def load_model(self) -> None:
        ...
        if not response.ok:
            ...
            raise RuntimeError(
                f"HTTP {response.status_code} when loading model from {url}. Response: {preview}"
            )
    ...
    def generate(self) -> None:
        ...
        try:
            ...
            if not response.ok:
                preview = (response.text or "").strip()[:500]
                raise RuntimeError(
                    f"HTTP {response.status_code} when calling {infer_url}. Response: {preview}"
                )
