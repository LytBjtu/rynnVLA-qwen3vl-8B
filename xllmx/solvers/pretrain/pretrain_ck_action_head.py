...
class PretrainSolverBase_ck_action_head(ABC):
    ...
    def __init__(self, args):
        ...
        fs_init.initialize_model_parallel(args.model_parallel_size)# 不再使用parallel，为了接口统一仍进行通信，使用FSDP进行训练优化
        ...
        self.dp_group = fs_init.get_data_parallel_group()# 获取通信组
        ...
        if self.args.auto_resume and self.args.resume_path is None:
            ...
            if len(existing_checkpoints) > 0:
                ...
                self.logger.info(f"auto resume from {self.args.resume_path}") # 取最新的epoch自动恢复训练
        ...
        # 训练集（分布式采样、预处理）、验证集同分布、验证集不同分布
        self.start_epoch = 0
        self.start_iter = 0
        self.metric_logger_to_resume = None

        # eval_only 评估 ft 微调 都不是恢复训练
    ...
    def build_model(self) -> Tuple[nn.Module, Tokenizer, torch.optim.Optimizer]:
        # 初始化来源
        ...
        # 定义训练参数：action_head、lm_head、lora等
        ...
        # 初始化checkpoint (part1, should be called before FSDP wrapping)
        ...
        # broadcast non-model-parallel parameters within model parallel group 广播所有的非并行参数
        misc.broadcast_nonmp_parameters(model)

        # checkpointing (part2, after FSDP wrapping) 应用梯度检查点
        if self.args.checkpointing:
        ...
        # Setup optimizer 初始化优化器，模型中所有可学习参数的迭代器，learning rate,权重衰减，AdamW动量一阶二阶矩
    ...
    @abstractmethod
    def _model_func(self, init_from: str) -> Tuple[nn.Module, Tokenizer | None]:  # todo return type get finer # noqa
    ...
    def setup_fsdp_sync(self, model: nn.Module, data_parallel: str, precision: str, grad_precision: Optional[str]) -> FSDP:
        ...
        model = FSDP(
            ...
            process_group=fs_init.get_data_parallel_group(),# 通信进程组
            sharding_strategy={
                "fsdp": ShardingStrategy.FULL_SHARD,
                "sdp": ShardingStrategy.SHARD_GRAD_OP,
            }[data_parallel],# 通常用fsdp，参数梯度优化器状态全分割
    ...
    def build_data(self):
    # 仅训练集
        eff_batch_size = self.args.batch_size * self.args.accum_iter * fs_init.get_data_parallel_world_size()# 单卡batch * 梯度积累步数 * 卡数
        ...
        sampler_train = FinetuneDistSampler(
            ...
            seed=self.args.seed,
            length_clustering=self.args.length_clustering, # 长度聚类分桶，减少padding消耗
    ...
    def build_data_new(self):
        ...
        self.logger.info("effective batch size: %d" % eff_batch_size)
        if self.args.preprocess=='true':# 是否token化
        ...
        self.logger.info(dataset_train)
        self.logger.info(dataset_val_ind) # 与训练集同分布的val
        self.logger.info(dataset_val_ood) # 泛化val
    ...
    def _unpack_model_outputs(self, outputs):
        if len(outputs) == 7:
            c_loss, additional_loss_dict, logits, hidden_states, labels_c, predicted_actions, loss_ct = outputs
        elif len(outputs) == 5:
            c_loss, additional_loss_dict, logits, hidden_states, labels_c = outputs
            predicted_actions = None
            loss_ct = c_loss * 0
        else:
            raise ValueError(f"Unexpected model outputs length: {len(outputs)}")
        return c_loss, additional_loss_dict, logits, hidden_states, labels_c, predicted_actions, loss_ct
    ...
    def _encode_action_tokens_qwen(self, action_value, start_token_id, end_token_id):
        ...
        # Keep compatibility with datasets that may store 6D/7D action.
        ...
        dim = min(len(action_value), len(min_values))
        action_value = action_value[:dim]
        min_values = min_values[:dim]
        max_values = max_values[:dim]
    ...
    def _build_qwen_examples_labels(self, conversations, images, actions, states):
        ...
        if processor is None:
            raise ValueError("Qwen processor is not initialized on model. Please check model setup.")

        tok = processor.tokenizer
        start_token = getattr(self.args, "action_start_token", "<|action_start|>")
        ...
        for conv, img, act, sta in zip(conversations, images, actions, states):
            ...
            user_prompt = user_prompt.replace("<|image|>", " ").replace("<|action|>", " ").replace("<|state|>", " ").strip()
            if self.args.with_state and sta is not None and len(np.array(sta).reshape(-1)) > 0:
            ...
            if len(cur_images) >= 2:
                ...
                qwen_images = [cur_images[-1]]

            content = [{"type": "text", "text": user_prompt}]
            for im in qwen_images:
                content.append({"type": "image", "image": im})
            content.append({"type": "text", "text": start_token})
    ...
    def _prepare_examples_labels(self, batch_data):
        ...
        if getattr(self.args, "vlm_arch", "chameleon") == "qwen3_vl":
            return self._build_qwen_examples_labels(conversations, images, actions, states)
